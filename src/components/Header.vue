<template>
  ...
  <router-link
    to="/cart"
    class="flex items-center text-white rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:hover:text-blue-700 md:p-0"
  >
    <!-- Cart Icon -->
    <svg
      xmlns="http://www.w3.org/2000/svg"
    ...
    <div v-if="totalItems > 0" class="absolute -top-2 -right-2 flex h-4 w-4 items-center justify-center rounded-full bg-red-500 text-xs text-white">
      {{ totalItems }}
    </div>
  </router-link>
</li>
...
// Computed properties for wishlist and cart item counts
